# from https://github.com/taku-buntu/Keras-DCGAN-killmebaby/blob/master/dcgan.py
...
class DCGAN():
    ...
    def load_imgs(self):
        ...
        num=0
        """
        for cnt,img_path in enumerate(tqdm(img_paths)):
            ...
            if (cnt+1) % 10000 == 0:
                ...
                # print(images.shape)
                tmp = []
                np.save('images_'+str(num),images)
                num +=1
            # images = np.append(images, img)
            # print(images.shape)
        # images = images.reshape(len(images)//256//256//4,256,256,4)
        images=np.array(tmp)
        
        np.save('images_'+str(num),images)
        """
        images = np.load('images_0.npy')
        for i in range(1,10):
            images = np.append(images,np.load('images_'+str(i)+'.npy'))
            # print(images.shape)
        np.reshape(images,(len(images)//self.shape[0]//self.shape[1]//self.shape[2],self.shape[0],self.shape[1],self.shape[2]))
