...
def Generative_Adversarial_Network(generator_model, discriminator_model):
    ...
    gen_optimizer = Adam(lr=1e-5, beta_1=0.1)
...
def main_train(z_input_size, generator_model, discriminator_model, gen_model, loss_dict, X_train,
               generated_figures=None, z_group=None, z_plot_freq=200, epoch=1000, plot_freq=25, batch=100):
    with tqdm(total=epoch) as pbar:
        for e in range(epoch):
            ...
            y = np.zeros(batch)
            y = y.astype(int)
            g_loss = gen_model.train_on_batch(x=noise, y=y)
            ...
            if e%plot_freq == plot_freq-1:
                ...
                gen_model.save('./model/gen_model_'+str(int(e/z_plot_freq))+'.h5')
                gen_model.save_weights('./model/gen_model_weights_'+str(int(e/z_plot_freq))+'.h5')

            # 訓練したgeneratorによる生成画像可視化
            if e < epoch:
...
main_train(100, Gen, Dis, GAN, loss_dict=gen_losses, X_train=X_train, generated_figures=generated_figures,
           z_group=z_group_matrix, z_plot_freq=z_plot_freq,epoch=epoch, plot_freq=plot_freq, batch=batch)
print("----Finished----")