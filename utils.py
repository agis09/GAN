...
class ImageLoader(object):
    ...
    def create_batch_pipeline(self):
        ...
        l = tf.maximum(r_size[0],r_size[1])
        single_image = tf.image.resize_image_with_crop_or_pad(single_image,l,l)
        nH,nW = self.cfg['input_shape'][:2]
        single_image = tf.cast(tf.image.resize_images(single_image,(nH,nW)),np.uint8)
        single_image.set_shape([nH, nW, int(single_image.shape[-1])])
        ...
        NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 64471
        ...
        image_batch = tf.train.shuffle_batch(
            ...
            batch_size=self.cfg['batch_size'],
            num_threads=16,
            capacity=min_queue_examples+3*self.cfg['batch_size'],
